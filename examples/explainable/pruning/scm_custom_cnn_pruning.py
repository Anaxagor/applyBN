...
transform = transforms.Compose([
    transforms.ToTensor(),
    transforms.Normalize((0.4914, 0.4822, 0.4465),  # Normalization parameters for CIFAR-10
...
class SimpleCNN(nn.Module):
    ...
    def forward(self, x):
        x = self.relu(self.conv1(x))  # 32x32x16
        x = self.pool(x)              # 16x16x16
        x = self.relu(self.conv2(x))  # 16x16x32
        x = self.pool(x)              # 8x8x32
        x = self.relu(self.conv3(x))  # 8x8x64
        x = self.pool(x)              # 4x4x64
...
logger.info('Custom CNN model instantiated and moved to device.')
...
random_accuracies = []
prune_percentages = [0, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50]
...
plt.title('Custom CNN Accuracy vs. Percentage of Filters Pruned')
...
# Collect all importance scores into a single list
all_importances = []
filter_labels = []
...
for idx in filter_importances.keys():
    ...
    for i in range(num_filters):
        all_importances.append(importance[i])
        filter_labels.append(f'Layer {idx} Filter {i}')

# Convert to numpy array for processing
all_importances = np.array(all_importances)

# Plot importance distribution across the network
plt.figure(figsize=(10, 6))
plt.hist(all_importances, bins=20, edgecolor='black')
plt.title('Importance Distribution Across the Network')
plt.xlabel('Importance Score')
plt.ylabel('Number of Filters')
plt.grid(True)
plt.show()

# Get top 5 filters by importance
top_indices = np.argsort(-all_importances)[:5]
top_importances = all_importances[top_indices]
top_filters = [filter_labels[i] for i in top_indices]

# Plot top 5 filters by importance
plt.figure(figsize=(8, 6))
plt.barh(top_filters[::-1], top_importances[::-1], color='green')
...
# Get top 5 filters with the lowest importance
bottom_indices = np.argsort(all_importances)[:5]
bottom_importances = all_importances[bottom_indices]
bottom_filters = [filter_labels[i] for i in bottom_indices]

# Plot top 5 filters with the lowest importance
plt.figure(figsize=(8, 6))
plt.barh(bottom_filters[::-1], bottom_importances[::-1], color='red')
plt.title('Top 5 Filters with the Lowest Importance')
plt.xlabel('Importance Score')
plt.ylabel('Filters')
plt.grid(True)
plt.show()
