...
class TestCausalCNNExplainer(unittest.TestCase):
    ...
    def test_initialization(self):
        ...
        self.assertTrue(len(explainer.conv_layers) > 0, "Should detect convolutional layers.")

    def test_collect_data(self):
        explainer = self.CausalCNNExplainer(model=self.mock_model, device=self.device)
        explainer.collect_data(self.mock_loader)
        self.assertGreater(len(explainer.filter_outputs), 0, "Should collect filter outputs.")
        self.assertTrue(all(k in explainer.filter_outputs for k in explainer.dag.keys() if k > 0 or k == 0),
    ...
    def test_learn_structural_equations(self):
        ...
        self.assertIsInstance(filter_importances, dict, "Filter importances should be stored in a dictionary.")
        ...
        for idx, importance in filter_importances.items():
            self.assertEqual(len(importance), explainer.dag[idx]["layer"].out_channels,
